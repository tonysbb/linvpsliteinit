...
- **后续可多次执行组件安装**：按需添加模块
- **内存策略**：可选择磁盘 SWAP、ZRAM 或两者；组件模式保留已有 swap
...
`vps_init.sh` 面向刚安装、尚未部署业务的纯净系统；`add_components.sh` 面向已初始化或已承载业务的系统，只修改用户选择的组件。ZRAM 是压缩内存 swap，不会增加物理内存；Chrony 只作为 NTP 客户端使用。
